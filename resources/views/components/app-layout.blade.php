    ...
    <div class="min-h-screen lg:flex">
        <aside class="hidden w-64 shrink-0 border-r border-slate-200 bg-white lg:flex lg:flex-col">
            ...
            <nav class="flex-1 space-y-1 p-4">
                @foreach ([['label' => 'Work', 'route' => 'work.index'], ['label' => 'Customers', 'route' => 'customers.index']] as $item)
                    <a href="{{ route($item['route']) }}" class="flex items-center rounded-xl px-3 py-2.5 text-sm font-medium {{ request()->routeIs($item['route']) ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950' }}">
                        {{ $item['label'] }}
                    </a>
                @endforeach
            ...
            <header class="border-b border-slate-200 bg-white">
                <div class="mx-auto max-w-7xl px-5 py-4 sm:px-8">
                    <div class="flex items-center justify-between gap-4">
                        <div class="min-w-0">
                            <a href="{{ route('work.index') }}" class="text-xl font-semibold tracking-tight text-slate-950 lg:hidden">
                                zazu<span class="text-sky-600">.</span>
                            </a>
                            <p class="mt-1 text-[11px] font-medium uppercase tracking-[0.14em] text-slate-400">Zazu EMP</p>
                            <h1 class="mt-1 truncate text-xl font-semibold tracking-tight text-slate-950">{{ $heading ?? $title ?? 'Workspace' }}</h1>
                        </div>
                        @isset($headerAction)
                            <div class="shrink-0">
                                {{ $headerAction }}
                    ...
                    <nav class="mt-4 flex gap-2 overflow-x-auto pb-1 lg:hidden" aria-label="Primary navigation">
                        @foreach ([['label' => 'Work', 'route' => 'work.index'], ['label' => 'Customers', 'route' => 'customers.index']] as $item)
                            <a href="{{ route($item['route']) }}" class="shrink-0 rounded-lg px-3 py-2 text-sm font-medium {{ request()->routeIs($item['route']) ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600' }}">
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    </nav>
